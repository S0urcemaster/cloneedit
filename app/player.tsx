...
export function Player() {
   ...
   return (
      <div className='player' style={{ display: 'flex', justifyContent: 'space-between', position: 'fixed', height: 20, background: settings.material, padding: '0px 0 0 0', borderBottom: `1px solid ${settings.materialMedian}`}}>
         <div style={{ display: 'flex', gap: 1, width: '100%' }}>
            ...
            <button style={{ height: 19, flexGrow: 1 }}><div style={{ marginTop: -1, fontSize: 'medium' }}>➄</div></button>
            {/* <button style={{ height: 19, flexGrow: 1 }}><div style={{ marginTop: -1, fontSize: 'medium' }}>➅</div></button> */}
            <button className={constants.fonts[constants.FONT_GEMUNU_LIBRE].font.className} style={{ whiteSpace: 'nowrap', paddingRight: 5, color: settings.brightColor, height: 19, flexGrow: 1, cursor: 'help' }}>Digi Playa</button>
         </div>
      </div>
   )
}